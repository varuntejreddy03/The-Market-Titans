...
import { ArrowRight, CheckCircle2, Globe, Clock, Shield, Star, Zap, DollarSign, Calendar, Users, Headphones, Briefcase } from 'lucide-react';
...
export default function RoleTemplate() {
  ...
  const role = categoryData?.roles.find(r => r.slug === slug);
  ...
  if (!role) {
    return (
      <div className="min-h-screen bg-[#0D1B2E] flex flex-col items-center justify-center text-white">
        <h1 className="text-4xl font-bold mb-4 font-display">Role Not Found</h1>
        <Link to="/roles" className="text-[#4DB8E8] hover:underline font-bold">Back to All Roles</Link>
      </div>
    );
  }
  ...
  return (
    <main key={slug} className="font-body bg-[#0D1B2E] min-h-screen relative z-10 overflow-hidden">
      ...
      <section className="relative pt-32 pb-12 px-6 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-8 reveal">
            <BackLink to={`/roles/${category}`} label={`BACK TO ${categoryData.category}`} />
            <span className="text-[#4DB8E8]/30 text-[10px]">|</span>
            <BackLink to="/roles" label="ALL DEPARTMENTS" />
          </div>
          <div className="max-w-4xl">
            <h1 className="font-display text-white font-bold mb-6 reveal" style={{ fontSize: 'clamp(32px, 4vw, 48px)', lineHeight: '1.2' }}>
              {role.headline}
            </h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
            <div className="reveal">
              <div className="flex items-center gap-4 mb-8">
                <Briefcase size={32} className="text-[#4DB8E8]" />
                <h2 className="font-display text-white font-bold text-3xl uppercase tracking-wider text-[#4DB8E8]">Excellence Defined</h2>
              </div>
              <div className="space-y-6">
                {role.paragraphs.slice(0, 4).map((p, i) => (
                  <p key={i} className="text-[#8899AA] text-lg leading-relaxed">
                    {p}
                  </p>
                ))}
              </div>
            ...
            <div className="reveal">
              <h2 className="font-display text-white font-bold text-3xl mb-8 uppercase tracking-wider text-[#F5A623]">Key Responsibilities</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {role.sub_titles.slice(0, 8).map((service, i) => (
      ...
      <section className="py-24 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16 reveal">
            <h2 className="font-display text-white font-bold text-4xl mb-4">Why Market Titans?</h2>
            <div className="w-24 h-1 bg-[#F5A623] mx-auto"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 reveal">
            {role.mt_trust_points.map((point, i) => {
      ...
      {/* STATS (4 CARDS) */}
      <section className="py-24 px-6 bg-[#0D1B2E] border-y border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 reveal">
            {role.mt_stats.map((stat, i) => {
      ...
      <section className="py-16 px-6 bg-[#0B1526]/50">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16 reveal">
            <h2 className="font-display text-white font-bold text-4xl mb-4">Flexible Recruitment Models</h2>
            <p className="text-[#8899AA]">Tailored offshore staffing solutions to fit your unique business needs.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 reveal">
            {role.mt_engagement.map((model, i) => (
      ...
      {role.faqs && role.faqs.length > 0 && (
        <section className="py-24 px-6">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-16 reveal">
              <h2 className="font-display text-white font-bold text-4xl mb-4">Common Questions</h2>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="bg-gradient-to-br from-[#1B2A4A] to-[#1E3A6E] rounded-[40px] p-16 text-center relative overflow-hidden reveal">
            ...
            <div className="relative z-10 max-w-3xl mx-auto">
              <h2 className="font-display text-white font-bold text-[32px] md:text-[56px] mb-6 leading-tight">
                {role.mt_cta.title}
              </h2>
              <p className="text-[#CBD5E0] text-xl mb-10">{role.mt_cta.subtitle}</p>
              <PrimaryButton to="/contact">{role.mt_cta.button} &rarr;</PrimaryButton>
            </div>
          </div>
        </div>
      </section>
